...
def main():
    ...
    if args.resume:
        ...
        checkpoint = torch.load(args.resume, map_location ='cuda', encoding='latin1', weights_only=False) # 'cpu' remove this when you have GPU
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        if (epoch + 1) > args.start_eval and args.eval_step > 0 and (epoch + 1) % args.eval_step == 0 or (
            ...
            time_name_save = time_stamp + '_' + args.name + '_'
            save_checkpoint({
                'state_dict': state_dict,
                'rank1': rank1,
                'epoch': epoch,
            }, is_best, time_name_save, osp.join(args.save_dir, 'checkpoint_ep' + str(epoch + 1) + time_name_save + '.pth.tar'))
